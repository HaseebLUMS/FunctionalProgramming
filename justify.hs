...
-- ALL DONE WITH 4 NIGHTS ON DESK, BUT LONG BEFORE DEADLINE FOR THE SAKE OF CONVINIENCE OF MIND..........
-- It works fine and I hope it to carry on. I have fought with numerous bugs in it and I tried my best to make it bug-free 
-- and I am sure to a good extent that it is bug-free.
...
splitLine (x) a = ((split x a 0), 
                  ((drop (length (split x a 0) )) (x)) )

split :: [String] -> Int -> Int -> [String]
split [] a z = []
split (x:xs) a z | ((length x) + z ) <= a  = [(x)] ++ split xs a ((length x) + z+1 ) 
                 | otherwise = []

--otherSplit [x] _ = [x]
--otherSplit (x:y:xs) z | x /= z = otherSplit (y:xs) z
--					| otherwise = [x] ++ otherSplit (y:xs) y


length' :: [String] -> Int
length' x = length (unwords x)
...
lineBreaks mp z (x) | length' (fst (splitLine x z )) /= z =  [(splitLine x z )]++filter (\q-> length'(fst q) < (z+1)) (lineBreaks' mp z x)
                    | otherwise = [(splitLine x z )]

lineBreaks' mp z x | (length' x) <= z = [splitLine x z]
                   | otherwise = lineBreaks'' mp z x
lineBreaks'' mp z (x) 
                     = let  a = fst (splitLine x z)
                            b = head (snd (splitLine x z) )
                            c = tail (snd (splitLine x z) )
                       in map (\q -> ( a++[fst q], [snd q]++c )) (makeTuples' b mp (length' a) z)
-- x is controls , mp is enHyp
makeTuples' x mp l z= makeTuples (realHyp x mp) l z
makeTuples x l z= [( 
               (if (concat (drop i x) == "") then (reverse (tail (reverse (auxString i x 0))))
               else  {-if (l + length' (auxString i x 0) +1 < z) then-} (auxString i x 0) ), (concat (drop i x)) )  | i<- [1 .. length x] ]
auxString i y a| y == [] || i > (length y ) || a > i  = ""
               | i == a = "-"
               | otherwise = (y !! a) ++ auxString i y (a+1)



--realHyp x mp = subHyp x mp (last x)
realHyp x (mp)  = appendPron (subHyp x mp (last x)) (last x)
subHyp x [] _= []
subHyp x (mp:mps) pron |    if (not(isAlpha pron)) 
                             then (reverse (tail (reverse x)) == (fst mp))
                             else ( x== (fst mp))
                             = (snd mp)
                       |    otherwise = subHyp x mps pron

appendPron [] pro = []
appendPron (x:xs) pro | isAlpha(pro) = (x:xs)
                      | otherwise && (xs == [] ) = [x ++ [pro]]
                      | otherwise = [x] ++ appendPron (xs) pro

-- Tuppeles are ready here-----------
...
-- Function definition here
blankInsertions z x | z < 0 = [x]
blankInsertions _ [] = []
blankInsertions _ [x] =[[x]]
blankInsertions z (x:y:xs) | xs == [] = [[x] ++ (replicate z " ") ++ [y]]
blankInsertions z (x:y:xs) =concat [map (\q -> [x] ++ (replicate a " ") ++ q) (blankInsertions (z-a) (y:xs) ) | a <- [0 .. z] ]
...
lineCost x = ( (blankCost * (auxBlank x)) + (blankProxCost * (auxBlankProx (auxList x) x)) + (blankUnevenCost * (auxBlankUnEven (auxList x))) + (hypCost * (auxHyp x)))
auxBlank :: [String] -> Double
auxBlank [] = 0.0
auxBlank (x:xs) | x == " " = 1.0 + auxBlank xs
                | otherwise = 0.0 + auxBlank xs

auxHyp :: [String] ->  Double
auxHyp (x) | last (last $ x) == '-' = 1.0
           | otherwise = 0.0
...
fsi (x:xs) | x == " " =0 
            |otherwise = 1+ fsi(xs)
--loraFunc [] = []
--loraFunc x = [fsi x] ++ (loraFunc (drop ((fsi x)+1) x))
auxList [] = []
auxList x = [fsi x] ++ (auxList (drop ((fsi x)+1) x))

average :: [Int] -> Double
average x = fromIntegral (div (sum x )  (length x))

--auxBlankProx :: [Int] -> Double 
auxBlankProx x origX= (fromIntegral(length origX) - ( average (x)))

auxBlankUnEven :: [Int] -> Double
auxBlankUnEven (x) = ( variance (x) )   

variance :: [Int] -> Double
variance (x) = ( sum (map (\q ->((average x)-(q))^2 ) (map fromIntegral x))) / (fromIntegral(length x))
...
bestLineBreak lineCost eHyp z x = (help2 x eHyp z) !! ((elemIndices (minimum (scoreList lineCost eHyp z x) ) (scoreList lineCost eHyp z x) ) !! 0)
--    lineBreaks enHyp 12 ["He", "who", "controls."]   ==>   [(["He","who"], ["controls."]), (["He","who","co-"], ["ntrols."]), (["He","who","cont-"], ["rols."])]
--    blankInsertions 2 ["A", "creative", "man"]   ==>   [["A", " ", " ", "creative", "man"], ["A", " ", "creative", " ", "man"], ["A", "creative", " ", " ", "man"]]


helpFunc :: [String] -> [(String,[String])] -> Int -> [[([String],[String])]]

helpFunc x (eHyp) z =  map (\q -> ( map (\r -> (r,snd q)) (blankInsertions (z-(length' (fst q))) (fst q) ) ) )  (lineBreaks eHyp z x)


help2 :: [String] -> [(String, [String])] -> Int -> [([String],[String])]

help2 x (eHyp) z = [((helpFunc x (eHyp) z) !! a) !! b | a <- [0 .. ((length (helpFunc x (eHyp) z) )-1)], b <- [0 .. ((length ((helpFunc x (eHyp) z) !! a))-1)]]

scoreList lineCost eHyp z x = map (\q -> lineCost (fst q) ) (help2 x eHyp z)
...
-- Function definition here
justifyText lineCost eHyp z text = setLast $ justifyText' lineCost eHyp z text
...
--  bestLineBreak lineCost enHyp 12 ["He", "who", "controls"]   ==>   (["He", "who", "cont-"], ["rols"])
justifyText' _ _ _ [] = []
justifyText' lineCost eHyp z text = jus' lineCost eHyp z (words text)

jus' _ _ _ [] = []
jus' lineCost eHyp z tex = [] ++ [auxunword (fst (bestLineBreak lineCost eHyp z tex))] ++ (justifyText' lineCost eHyp z (unwords(snd (bestLineBreak lineCost eHyp z tex))))

setLast :: [String] -> [String]
setLast xs = let a = reverse xs
                 b  = unwords $ words $ head a
             in  reverse ([b] ++ (tail a) )
...
auxunword [] = []
auxunword [x] = x
auxunword (x:xs) | x /= " " = x ++ " " ++  ( auxunword xs ) 
                  | otherwise = x ++ ( auxunword xs )
